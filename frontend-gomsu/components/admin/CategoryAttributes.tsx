...
export default function CategoryAttributes({ categoryId }: CategoryAttributesProps) {
  ...
  async function add(e: FormEvent) {
    ...
    setError('');
    try {
      const res = await authFetch<CategoryAttribute>(`/categories/${categoryId}/attributes`, {
        method: 'POST',
        body: JSON.stringify({ attributeLabel: label, attributeType: type }),
      });
      if (res.data) {
        setRows((prev) =>
          [...prev, res.data].sort((a, b) => a.attributeLabel.localeCompare(b.attributeLabel))
        );
      }
      setLabel('');
    } catch (err: unknown) {
  ...
  async function remove(id: string, name: string) {
    if (
      !confirm(
        `Xoá thông số "${name}"? Giá trị đã nhập ở các sản phẩm vẫn được giữ, chỉ thôi hiển thị.`
      )
    )
      return;
    try {
      ...
      alert('Lỗi khi xoá: ' + (err instanceof Error ? err.message : 'Không xác định'));
    }
  }
    ...
    <div className="mt-3 pt-3 border-t border-gray-100">
        ...
        <ul className="flex flex-wrap gap-2 mb-3">
          {rows.map((r) => (
            <li key={r.id} className="flex items-center gap-2 bg-gray-100 rounded px-2.5 py-1 text-xs">
              ...
              <button
                type="button"
                onClick={() => remove(r.id, r.attributeLabel)}
      ...
      <form onSubmit={add} className="flex gap-2">
        ...
        <button
          type="submit"
          disabled={busy}
          className="bg-gray-800 text-white rounded px-4 py-1.5 text-sm disabled:opacity-50"
        >
          Thêm
        </button>
      </form>
