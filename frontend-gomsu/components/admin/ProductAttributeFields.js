...
export default function ProductAttributeFields({ categoryId, values = {}, onChange }) {
    ...
    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
      {defs.map((def) => (
        <div key={def.id} className="flex flex-col gap-1.5">
          <label className="text-xs uppercase tracking-wider text-gray-500">{def.attributeLabel}</label>
          <input
