...
export default function ProductSpecifications({ product, shopSpecs = {} }: ProductSpecificationsProps) {
  const [activeTab, setActiveTab] = useState<'specs' | 'placement' | 'care'>('specs');
  ...
  const perProduct = (product.category?.attributes || [])
    .map((def) => ({
      label: def.attributeLabel,
      value: product.attributes?.[def.attributeKey] as string | number | undefined | null,
    }))
    ...
    <section className="border-t border-gomsu-border py-16 bg-[#111111]">
      <div className="page-shell">
        <div className="text-center mb-10">
          ...
          <p className="text-xs uppercase tracking-widest text-gomsu-text-muted">
            Cung cấp dữ liệu chi tiết minh bạch cho quyết định mua hàng
          </p>
        </div>
        ...
        <div className="bg-[#1a1a1a] border border-[#2a2a2a] p-6 md:p-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {specs.map((item, idx) => (
                <div key={idx} className="flex justify-between py-3 border-b border-[#2a2a2a] text-xs">
                  ...
                  <span className="text-white font-medium text-right">{String(item.value)}</span>
                </div>
              ))}
            </div>
          </div>
          ...
          <div role="tabpanel" hidden={activeTab !== 'placement'}>
            <div className="space-y-4 text-xs text-gomsu-text-muted leading-relaxed">
              ...
              <ul className="list-disc pl-5 space-y-2">
                <li>
                  <strong className="text-gomsu-primary font-normal">Phòng khách & Kệ Tivi:</strong> Đặt ở nơi có ánh sáng rọi nhẹ (Warm Light 3000K) để tôn vinh vệt men lấp lánh.
                </li>
                <li>
                  <strong className="text-gomsu-primary font-normal">Trà thất & Không gian Thiền:</strong> Kết hợp cùng bàn gỗ trầm, mây bối hoặc hoa sen khô tạo không gian thanh tịnh.
                </li>
                <li>
                  <strong className="text-gomsu-primary font-normal">Bàn làm việc Chủ tịch:</strong> Tác phẩm mang năng lượng thủy thổ hòa hợp, giúp tĩnh tâm và nâng tầm đẳng cấp không gian làm việc.
                </li>
              </ul>
            </div>
          </div>
          ...
          <div role="tabpanel" hidden={activeTab !== 'care'}>
            <div className="space-y-4 text-xs text-gomsu-text-muted leading-relaxed">
              ...
              <ul className="list-disc pl-5 space-y-2">
                <li>
                  Lau chùi bằng khăn mềm ẩm hoặc chổi lông thỏ mịn. Không dùng búi sắt hoặc chất tẩy rửa có tính axit mạnh.
                </li>
                <li>
                  Sản phẩm đã qua xử lý nung cao độ, bề mặt men kháng nước tuyệt đối, không bám ố dơ theo thời gian.
                </li>
