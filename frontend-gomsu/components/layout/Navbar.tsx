      ...
      <div className="page-shell flex items-center justify-between py-5">
        ...
        <Link href="/" className="flex items-center gap-3" onClick={() => setIsOpen(false)}>
          ...
          <div className="flex flex-col justify-center">
            <span className="font-serif text-2xl tracking-wide text-gomsu-primary leading-none mb-1">
              Nghĩa Phái
            </span>
            <span className="font-sans text-[10px] uppercase tracking-widest text-gomsu-text-muted leading-none">
              Hơi thở Bát Tràng
            </span>
          </div>
        </Link>
        ...
        <div className="flex items-center gap-6">
          ...
          {/* Danh sách tư vấn — hiện cho cả khách chưa đăng nhập, vì luồng này không cần tài khoản */}
          {count > 0 && (
            <Link
              href="/yeu-cau-tu-van"
              className="relative text-xs uppercase tracking-widest text-gomsu-primary hover:text-white transition-colors whitespace-nowrap"
            >
              Yêu cầu tư vấn
              <span className="absolute -top-2 -right-4 bg-gomsu-primary text-black rounded-full w-5 h-5 flex items-center justify-center text-[10px] font-bold">
                {count}
              </span>
            </Link>
          )}
          ...
          {user ? (
            <div className="hidden md:flex items-center gap-4">
              ...
              >
                {user.role === 'admin' ? (
                  <span className="bg-gomsu-primary text-black px-2 py-0.5 rounded-sm font-bold tracking-widest text-[10px]">
                    ADMIN
                  </span>
                ) : null}
            ...
            <div className="hidden md:flex items-center gap-3">
              <Link
                href="/auth/login"
                className="text-xs uppercase tracking-widest text-gomsu-text-muted hover:text-gomsu-primary transition-colors"
              >
                Đăng nhập
              </Link>
              <Link
                href="/auth/register"
                className="text-xs uppercase tracking-widest border border-gomsu-primary text-gomsu-primary px-3 py-1.5 hover:bg-gomsu-primary hover:text-black transition-colors"
              >
                Đăng ký
              </Link>
            </div>
          ...
          >
            ...
            ></span>
            <span
              className={`w-6 h-[1px] bg-gomsu-text transition-opacity ${
                isOpen ? 'opacity-0' : ''
              }`}
            ></span>
            <span
