...
export default async function AboutPage() {
  ...
  try {
    const res = await apiFetch<Record<string, string>>('/settings?keys=about_background,about_portrait', { cache: 'no-store' });
    settings = res?.data || {};
  } catch (error) {
    const msg = error instanceof Error ? error.message : String(error);
    console.error('Lỗi khi tải cấu hình trang Về chúng tôi:', msg);
  }
