...
export default function SettingsPage() {
  ...
  const [specs, setSpecs] = useState({ product_spec_clay: '', product_spec_glaze: '', product_spec_safety: '' });
  const [savingSpecs, setSavingSpecs] = useState(false);
  const [specsMsg, setSpecsMsg] = useState(null);

  useEffect(() => {
    authFetch('/settings?keys=product_spec_clay,product_spec_glaze,product_spec_safety')
  ...
  async function saveSpecs(e) {
    ...
    try {
      ...
      setSpecsMsg('Lỗi khi lưu: ' + err.message);
    } finally {
