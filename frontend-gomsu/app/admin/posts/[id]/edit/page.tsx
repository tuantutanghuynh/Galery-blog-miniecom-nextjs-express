...
export default function EditPostPage({ params }: PageProps) {
  ...
  const [form, setForm] = useState<EditPostFormState | null>(null); // null = đang tải dữ liệu cũ
  const [submitting, setSubmitting] = useState(false);
  const [errorMsg, setErrorMsg] = useState<string | null>(null);

  useEffect(() => {
    authFetch<BlogPost[]>(`/blog/admin/list?categorySlug=${BRAND_CATEGORY_SLUG}`).then((res) => {
  ...
  async function handleCoverUpload(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      setForm((f) => f ? { ...f, coverImageUrl: url } : null);
    } catch {
      ...
      <form onSubmit={handleSubmit} className="flex flex-col gap-6">
        ...
        <div className="flex flex-col gap-2 mt-4">
          ...
          <div className="admin-light-editor-wrapper">
            <RichTextEditor
              value={form.content}
              onChange={(content) => setForm((f) => f ? ({ ...f, content }) : null)}
            />
          </div>
        </div>
