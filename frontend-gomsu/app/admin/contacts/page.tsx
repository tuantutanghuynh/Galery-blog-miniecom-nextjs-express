...
export default function AdminContactsPage() {
  ...
  const [selectedContact, setSelectedContact] = useState<ContactMessage | null>(null); // Để hiển thị modal chi tiết

  const loadContacts = async () => {
    try {
      const res = await authFetch<ContactMessage[]>('/contact/admin/list?pageSize=50');
  ...
  const handleMarkAsRead = async (id: string, e: React.MouseEvent | { stopPropagation: () => void }) => {
    e.stopPropagation();
    try {
