...
export default function EditProductPage({ params }: PageProps) {
  ...
  const [form, setForm] = useState<ProductFormState>({ name: '', slug: '', description: '', status: 'draft', categoryId: '', attributes: {} });
  const [variants, setVariants] = useState<VariantEditItem[]>([]);
  const [images, setImages] = useState<ProductImage[]>([]);
  const [newVariant, setNewVariant] = useState<NewVariantFormState>({ sku: '', label: '', price: 0, compareAtPrice: '', stockQuantity: 0 });
  ...
  useEffect(() => {
    authFetch<Category[]>('/categories').then((res) => {
      const all = res.data || [];
    ...
    authFetch<Product[]>(`/products/admin/list?categorySlug=${BRAND_CATEGORY_SLUG}&pageSize=200`).then((res) => {
      ...
      setVariants(found.variants.map((v) => ({
        id: v.id,
        sku: v.sku,
        label: v.variantAttributes?.label ? String(v.variantAttributes.label) : '',
  ...
  // admin vừa nhìn vào.
  async function handleSaveVariant(v: VariantEditItem) {
    ...
    try {
      ...
      const msg = err instanceof Error ? err.message : 'Lỗi khi lưu biến thể';
      setErrorMsg(msg);
    } finally {
  ...
  async function handleUploadImages(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    try {
      for (const file of files) {
        ...
        const up = await authFetch<{ url: string }>('/uploads/image', { method: 'POST', body: formData });
        if (!up.data?.url) throw new Error(up.error?.message || 'Tải ảnh thất bại');

        const saved = await authFetch<ProductImage>(`/products/${id}/images`, {
          method: 'POST',
  ...
  async function handleSaveAlt(image: ProductImage) {
    try {
      ...
      const msg = err instanceof Error ? err.message : 'Lỗi lưu mô tả';
      setErrorMsg(msg);
    }
  }
  ...
  async function handleSetPrimary(image: ProductImage) {
    ...
    try {
      const json = await authFetch<ProductImage>(`/products/images/${image.id}`, {
        method: 'PATCH',
      ...
      if (json.data) {
        ...
        setImages((prev) =>
          [...prev.map((i) => (i.id === image.id ? updated : i))].sort((a, b) => a.position - b.position)
        );
      }
      flash('Đã đặt làm ảnh bìa.');
    } catch (err) {
      const msg = err instanceof Error ? err.message : 'Lỗi đặt ảnh bìa';
      setErrorMsg(msg);
    }
  }
  ...
  async function handleDeleteImage(image: ProductImage) {
    ...
    try {
      ...
      const msg = err instanceof Error ? err.message : 'Lỗi xoá ảnh';
      setErrorMsg(msg);
    }
  }

  async function handleDeleteVariant(v: VariantEditItem) {
    ...
    try {
      ...
      const msg = err instanceof Error ? err.message : 'Lỗi xoá biến thể';
      setErrorMsg(msg);
    }
  }
  ...
  async function handleAddVariant(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      interface CreatedVariantResponse {
        id: string;
        sku: string;
        variantAttributes?: { label?: string };
        price: number;
        compareAtPrice?: number | null;
        stockQuantity: number;
      }
      const json = await authFetch<CreatedVariantResponse>(`/products/${id}/variants`, {
        method: 'POST',
      ...
      if (json.data) {
        setVariants((prev) => [
          ...prev,
          {
            id: json.data.id,
            sku: json.data.sku,
            label: json.data.variantAttributes?.label ? String(json.data.variantAttributes.label) : '',
      ...
      const msg = err instanceof Error ? err.message : 'Lỗi thêm biến thể';
      setErrorMsg(msg);
    } finally {
      ...
      <form onSubmit={handleSaveInfo} className="bg-white border border-gray-200 rounded-lg shadow-sm p-6 flex flex-col gap-5 mb-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
          ...
          <div>
            ...
            <select value={form.status} onChange={(e) => setForm({ ...form, status: e.target.value as ProductStatus })} className={inputCls + ' bg-white'}>
