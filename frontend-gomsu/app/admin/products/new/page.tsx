...
const emptyVariant = (): VariantFormItem => ({ label: '', sku: '', price: '', stockQuantity: '' });
...
export default function NewProductPage() {
  ...
  const [variants, setVariants] = useState<VariantFormItem[]>([emptyVariant()]);
  ...
  function updateVariant(index: number, field: keyof VariantFormItem, value: string) {
  ...
  async function handleUpload(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    try {
      for (const file of files) {
        ...
        const json = await authFetch<{ url: string }>('/uploads/image', { method: 'POST', body: formData });
      ...
      setErrorMsg('Lỗi tải ảnh lên: ' + msg);
    } finally {
      setUploading(false);
        ...
        <section className="bg-white border border-gray-200 rounded-lg shadow-sm p-6 flex flex-col gap-5">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
            ...
            <div>
              ...
              <select value={status} onChange={(e) => setStatus(e.target.value as ProductStatus)} className={inputCls + ' bg-white'}>
