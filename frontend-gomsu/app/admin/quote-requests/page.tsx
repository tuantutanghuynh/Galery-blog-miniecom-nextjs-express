...
import { QuoteRequest, QuoteRequestItem, QuoteStatus } from '@/types/quote';
...
export default function AdminQuoteRequestsPage() {
  ...
  const load = async (status: string) => {
    ...
    try {
      ...
      const res = await authFetch<QuoteRequest[]>(`/quote-requests/admin/list?pageSize=50${qs}`);
      setRows(res.data || []);
    } catch (error) {
      const msg = error instanceof Error ? error.message : 'Lỗi không xác định';
      alert('Không tải được danh sách yêu cầu: ' + msg);
    } finally {
      setLoading(false);
    }
  };
  ...
  useEffect(() => {
    authFetch<QuoteStats>('/quote-requests/admin/stats')
      .then((res) => setStats(res.data || null))
      .catch(() => setStats(null));
  }, [rows.length]);

  const changeStatus = async (id: string, status: QuoteStatus, finalAmount?: number) => {
    try {
      const res = await authFetch<QuoteRequest>(`/quote-requests/admin/${id}/status`, {
        method: 'PATCH',
    ...
    <div>
      ...
      {stats && (
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
          {[
            { label: 'Tổng yêu cầu', value: stats.totalRequests, accent: false },
            { label: 'Đã chốt', value: stats.closedCount, accent: false },
            { label: 'Doanh thu', value: formatPrice(stats.revenue), accent: true },
            { label: 'Tỉ lệ chốt', value: `${stats.conversionRate}%`, accent: false },
          ].map((s) => (
