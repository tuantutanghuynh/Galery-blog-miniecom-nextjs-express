...
export default function EditGalleryItemPage({ params }: PageProps) {
  ...
  // Unwrap params using React.use() to avoid Next.js 15+ warnings
  ...
  useEffect(() => {
    // Tải danh mục và dữ liệu ảnh cùng lúc
    Promise.all([
      authFetch<Category[]>('/categories'),
      authFetch<GalleryItem>(`/gallery/${id}`)
    ]).then(([catRes, itemRes]) => {
      if (catRes.data) {
        const brand = catRes.data.find(c => c.slug === process.env.NEXT_PUBLIC_BRAND_CATEGORY_SLUG);
        if (brand) {
          const filtered = catRes.data.filter(c => c.id === brand.id || c.parentId === brand.id);
      ...
      if (itemRes.data) {
        setTitle(itemRes.data.title || '');
        setAltText(itemRes.data.altText || '');
        setImageUrl(itemRes.data.imageUrl || '');
        setCategoryId(itemRes.data.categoryId || '');
      } else {
        setErrorMsg('Không tìm thấy ảnh này.');
      }
      setIsLoading(false);
    }).catch(() => {
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    setErrorMsg(null);
    try {
      await authFetch(`/gallery/${id}`, {
        method: 'PUT',
        body: JSON.stringify({ title, altText, imageUrl, categoryId }),
      });
      router.push('/admin/gallery');
    } catch {
      setErrorMsg('Lỗi cập nhật ảnh Gallery.');
    } finally {
      setSubmitting(false);
    }
  }
  ...
  return (
    <div className="max-w-2xl mx-auto mt-8 px-4">
      <h1 className="text-3xl font-serif text-gray-900 tracking-wide mb-8">Sửa ảnh Gallery</h1>
      ...
      <form onSubmit={handleSubmit} className="flex flex-col gap-6">
        ...
        <div className="flex flex-col gap-2 p-6 border border-gray-200 bg-gray-50 rounded">
          <label className="text-sm uppercase tracking-widest text-gray-500 font-medium">Thay đổi ảnh (Tùy chọn)</label>
