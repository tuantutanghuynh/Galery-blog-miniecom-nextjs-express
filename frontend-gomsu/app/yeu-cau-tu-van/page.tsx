...
export default function QuoteRequestPage() {
  ...
  const [website, setWebsite] = useState<string>(''); // honeypot, người thật không thấy trường này
  const [status, setStatus] = useState<StatusState>({ busy: false, error: '', sent: false });
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    try {
      ...
      setStatus({ busy: false, error: errorMsg, sent: false });
    }
  }
  ...
  if (status.sent) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center px-6 text-center gap-5">
        <div className="w-16 h-16 border border-gomsu-primary text-gomsu-primary rounded-full flex items-center justify-center text-2xl">
          ✓
        </div>
    ...
    return (
      <div className="min-h-screen flex flex-col items-center justify-center px-6 text-center gap-5">
        ...
        <p className="text-sm text-gomsu-text-muted">
          Chọn sản phẩm bạn quan tâm, chúng tôi sẽ gọi lại tư vấn.
        </p>
        <Link href="/san-pham" className="text-sm text-gomsu-primary hover:underline">
          Xem sản phẩm
        </Link>
  ...
  return (
    <div className="min-h-screen page-shell py-12">
      <nav className="text-xs uppercase tracking-widest text-gomsu-text-muted mb-8">
        <Link href="/" className="hover:text-gomsu-primary">
          Trang chủ
        </Link>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
        ...
        <div className="lg:col-span-2">
              ...
              <div key={item.variantId} className="flex gap-4 p-4 items-center">
                ...
                <div className="flex-1 min-w-0">
                  <Link
                    ...
                    className="font-serif text-base hover:text-gomsu-primary"
                  >
                    {item.productName}
                  </Link>
                ...
                <div className="flex items-center border border-gomsu-border shrink-0">
                  <button
                    ...
                    className="px-3 py-1 hover:text-gomsu-primary"
                  >
                    −
                  </button>
          ...
          <Link
            href="/san-pham"
            className="inline-block mt-6 text-sm text-gomsu-primary hover:underline"
          >
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div>
              ...
              <textarea
                ...
                rows={3}
                maxLength={500}
                placeholder="Thời gian tiện gọi, yêu cầu riêng..."
