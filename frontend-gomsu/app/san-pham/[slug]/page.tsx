...
async function getProduct(slug: string): Promise<Product | null> {
  try {
    const res = await apiFetch<Product>(`/products/${slug}`, { cache: 'no-store' });
    return res.data;
  } catch {
    return null;
  }
...
export default async function ProductDetailPage({ params }: PageProps) {
  ...
  const [product, settings] = await Promise.all([
    ...
    apiFetch<ShopSettings>(
      '/settings?keys=product_story_image,product_spec_clay,product_spec_glaze,product_spec_safety',
      { cache: 'no-store' }
    )
      .then((r) => r?.data || {})
      .catch(() => ({} as ShopSettings)),
  ]);
  ...
  const jsonLd = {
    ...
    material:
      [settings.product_spec_clay, settings.product_spec_glaze].filter(Boolean).join(', ') ||
      undefined,
    offers:
      prices.length > 1
        ? {
        ...
        ? [
            {
              ...
              item: `${siteUrl}/san-pham?category=${product.category.slug}`,
            },
          ]
        : []),
    ...
    <article>
      ...
      <nav className="border-b border-gomsu-border text-xs uppercase tracking-widest text-gomsu-text-muted">
        <div className="page-shell py-5">
          ...
          <Link href="/san-pham" className="hover:text-gomsu-primary">
            Sản phẩm
          </Link>
          {product.category && (
            <>
              ...
              <Link
                ...
                className="hover:text-gomsu-primary"
              >
      ...
      <div className="page-shell grid grid-cols-1 lg:grid-cols-2 border-b border-gomsu-border">
        ...
        <div className="p-8 lg:p-12 flex flex-col gap-8">
          ...
          {product.description && (
            <div className="text-gomsu-text-muted leading-relaxed whitespace-pre-line">
              {product.description}
            </div>
          )}
                  ...
                  {product.variants.map((v) => {
                    ...
                    return (
                      <tr key={v.id} className="border-b border-gomsu-border/50">
                        <td className="py-3">
                          {(v.variantAttributes?.label as string) || v.sku}
                        </td>
      ...
      <ProductStory
        product={product}
        imageUrl={settings.product_story_image || '/images/about/nghia-profile.jpg'}
      />
